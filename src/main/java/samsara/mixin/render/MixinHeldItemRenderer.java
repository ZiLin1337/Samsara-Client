...
public abstract class MixinHeldItemRenderer implements IAccess {
    ...
    private void onUpdateHeldItems(CallbackInfo ci) {
        ScaffoldRecodeModule scaffold = Samsara.getInstance().getModuleManager().getModule(ScaffoldRecodeModule.class);
    ...
    private void hookRenderFirstPersonItem(AbstractClientPlayer player, float tickProgress, float pitch, InteractionHand hand, float swingProgress, ItemStack item, float equipProgress, PoseStack matrices, SubmitNodeCollector orderedRenderCommandQueue, int light, CallbackInfo ci) {
        if (InteractionHand.MAIN_HAND == hand && Samsara.getInstance().getModuleManager().getModule(CameraModule.class).isToggled()) {
            matrices.translate(
    ...
    private void hideShield(AbstractClientPlayer player, float tickProgress, float pitch, InteractionHand hand, float swingProgress, ItemStack item, float equipProgress, PoseStack matrices, SubmitNodeCollector orderedRenderCommandQueue, int light, CallbackInfo ci) {
        final var module = Samsara.getInstance().getModuleManager().getModule(AnimationModule.class);
    ...
    private float injectDisableEquipOffset(float y) {
        return AnimationModule.disableItemSwingOffset.getProperty() && Samsara.getInstance().getModuleManager().getModule(AnimationModule.class).isToggled()
                ? ITEM_POS_Y : y;
    }
    ...
    private ItemUseAnimation hookUseAction(ItemStack instance) {
        ...
        if (mc.player.getInventory().getSelectedItem().is(ItemTags.SWORDS) && (Samsara.getInstance().getModuleManager().getModule(KillauraModule.class).isToggled() &&
                Samsara.getInstance().getModuleManager().getModule(KillauraModule.class).shouldRenderFakeAnim() ||
                Minecraft.getInstance().options.keyUse.isDown()) && Samsara.getInstance().getModuleManager().getModule(AnimationModule.class).isToggled()) {
    ...
    private boolean hookIsUseItem(AbstractClientPlayer instance) {
        ...
        if (mc.player.getInventory().getSelectedItem().is(ItemTags.SWORDS) && (Samsara.getInstance().getModuleManager().getModule(KillauraModule.class).isToggled() &&
                Samsara.getInstance().getModuleManager().getModule(KillauraModule.class).shouldRenderFakeAnim() || Minecraft.getInstance().options.keyUse.isDown()) && Samsara.getInstance().getModuleManager().getModule(AnimationModule.class).isToggled()) {
            return true;
        }
    ...
    private float applyEquipOffset(float equipProgress) {
        if (Samsara.getInstance().getModuleManager().getModule(AnimationModule.class).isToggled()) {
            return 0.0F;
        }
    ...
    private void transformLegacyBlockAnimations(AbstractClientPlayer player, float tickProgress, float pitch, InteractionHand hand, float swingProgress, ItemStack item, float equipProgress, PoseStack matrices, SubmitNodeCollector orderedRenderCommandQueue, int light, CallbackInfo ci) {
        if (Samsara.getInstance().getModuleManager().getModule(AnimationModule.class).isToggled() && mc.player.getInventory().getSelectedItem().is(ItemTags.SWORDS)) {
