...
public class EncryptionUtil {
    ...
    public static byte[] generateDynamicKey(long seed) {
        try {
            ...
            String input = Base64.getEncoder().encodeToString(cc.samsara.protection.util.Base64.decode("r07JdQhUpCHIhfNns8R4TKIOEtBng3wI1J8uRrFUPuA="))
                    + "|" + seed;
    ...
    static {
        ...
        try {
            PRIVATE_KEY = KeyFactory.getInstance("RSA").generatePrivate(new PKCS8EncodedKeySpec(cc.samsara.protection.util.Base64.decode(privateKey)));
        } catch (Exception e) {
    ...
    public static String decrypt(String base64, byte[] key, byte[] aad) {
        try {
            byte[] ct = cc.samsara.protection.util.Base64.decode(base64);
