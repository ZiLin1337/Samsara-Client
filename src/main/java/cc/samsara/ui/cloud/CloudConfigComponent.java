...
import cc.samsara.util.client.ConfigUtil;
...
public class CloudConfigComponent {
    ...
    public void render(int mouseX, int mouseY) {
        ...
        SkijaUtil.roundedRectangle(buttonX, buttonY, buttonSize, buttonSize, 8, hovered ? Samsara.getInstance().getFirstColor() : new Color(30, 30, 30));
        ...
        if (animProgress > 0.01f || !windowAnimation.isFinished()) {
            ...
            SkijaUtil.roundedRectangleGradientVarying(windowX, windowY, rectWidth, 12, 8, 8, 0, 0, Samsara.getInstance().getFirstColor(), Samsara.getInstance().getSecondColor(), false);
            ...
            SkijaUtil.roundedRectangle(uploadButtonX, uploadButtonY, uploadButtonWidth, uploadButtonHeight, 4, new Color(Samsara.getInstance().getFirstColor().getRed(), Samsara.getInstance().getFirstColor().getGreen(), Samsara.getInstance().getFirstColor().getBlue(), 200));
    ...
    private void renderUploadDialogAnimated() {
        ...
        SkijaUtil.roundedRectangleGradientVarying(currentX, currentY, currentWidth, 12 * scale, 8 * scale, 8 * scale, 0, 0, Samsara.getInstance().getFirstColor(), Samsara.getInstance().getSecondColor(), false);
        ...
        Color nameFieldColor = (activeInputField == 0) ? Samsara.getInstance().getFirstColor() : new Color(50, 50, 50);
        ...
        Color descFieldColor = (activeInputField == 1) ? Samsara.getInstance().getFirstColor() : new Color(50, 50, 50);
        ...
        SkijaUtil.roundedRectangle(uploadButtonX, fieldY, buttonWidth, buttonHeight, 4 * scale, new Color(Samsara.getInstance().getFirstColor().getRed(), Samsara.getInstance().getFirstColor().getGreen(), Samsara.getInstance().getFirstColor().getBlue(), 200));
    ...
    private void renderConfigCard(ICloudConfig config, float x, float y, float width, float height) {
        ...
        SkijaUtil.roundedRectangle(x, y, 3, height, 6, Samsara.getInstance().getFirstColor());

        float textX = x + 12, textY = y + 12;
        Fonts.product_bold_11.drawString(config.getName(), textX, textY, Color.WHITE);

        textY += 16;
        Fonts.product_regular_10.drawString(config.getUser().getName(), textX, textY, Samsara.getInstance().getFirstColor());
        ...
        SkijaUtil.roundedRectangle(buttonX, buttonY, buttonWidth, buttonHeight, 3, new Color(Samsara.getInstance().getFirstColor().getRed(), Samsara.getInstance().getFirstColor().getGreen(), Samsara.getInstance().getFirstColor().getBlue(), 180));
    ...
    public boolean mouseClicked(MouseButtonEvent click, boolean doubled) {
        ...
        if (button == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
            ...
            if (mouseX >= buttonX && mouseY >= buttonY && mouseX <= buttonX + buttonSize && mouseY <= buttonY + buttonSize) {
                visible = !visible;
                if (visible) {
                    new FetchCloudConfigsPacket().sendPacket(Samsara.getInstance().getClient());
                } else {
            ...
            for (int i = 0; i < Flags.cloudConfigs.size(); i++) {
                ...
                if (mouseX >= downloadButtonX && mouseX <= downloadButtonX + buttonWidth &&
                    ...
                    FileUtil.writeJsonToFile(config.getConfigData(), cfgFile.getAbsolutePath() + "/" + config.getName() + ".samsara");
                    return true;
                }
            }
    ...
    private boolean handleUploadDialogClick(double mouseX, double mouseY) {
        ...
        if (mouseX >= uploadButtonX && mouseX <= uploadButtonX + 80 * scale && mouseY >= fieldY && mouseY <= fieldY + buttonHeight) {
            ...
            )).sendPacket(Samsara.getInstance().getClient());
            new FetchCloudConfigsPacket().sendPacket(Samsara.getInstance().getClient());
            showUploadDialog = false;
            return true;
        }
