package cc.samsara.ui.screens.clickgui.dropdown.impl;
...
import cc.samsara.ui.screens.clickgui.dropdown.impl.properties.*;
import cc.samsara.ui.screens.clickgui.dropdown.Component;
        ...
        if (!module.isToggled()) {
            ...
            if (shouldBeRounded) {
                SkijaUtil.roundedRectangleGradientVarying(getX(), getY(), getWidth(), getHeight(),
                        0, 0, 5, 5,
                        ColorUtil.withAlpha(Samsara.getInstance().getFirstColor(), baseAlpha),
                        ColorUtil.withAlpha(Samsara.getInstance().getSecondColor(), baseAlpha),
                        false
                );
            } else {
        ...
        switch (property) {
            ...
            default ->
                /* we could through an exception but since there are multiple properties I didn't do yet, so I will add it later */
                    Samsara.LOGGER.error("Unknown Property! " + property.getName());
        }
    }
}