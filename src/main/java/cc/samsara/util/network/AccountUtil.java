...
import cc.samsara.util.io.ThreadUtil;
...
public class AccountUtil implements IAccess {
    public static void loginWithCooke(File path, boolean addAlt) {
        ThreadUtil.runAsync(() -> {
            ...
            try {
                if (!path.exists()) {
                    Samsara.LOGGER.warn("Selected file does not exist.");
                    return;
                }
                ...
                try {
                    ...
                    Samsara.LOGGER.error("Failed to read cookie file", e);
                    return;
                }
                ...
                if (mcRes == null) {
                    Samsara.LOGGER.error("Failed to authenticate with Xbox Live");
                    return;
                }
                ...
                if (profileRes == null) {
                    Samsara.LOGGER.error("Failed to get Minecraft profile");
                    return;
                }
    ...
    private static void closeQuietly(Closeable closeable) {
        if (closeable != null) {
            try {
                ...
                Samsara.LOGGER.debug("Error closing resource", e);
            }
        }
    }
    ...
    public static long getTokenExpirationTime(String token) {
        try {
            ...
            String payload = new String(cc.samsara.protection.util.Base64.decode(parts[1]), StandardCharsets.UTF_8);
