        ...
        try {
            ...
            if (responseCode == HttpURLConnection.HTTP_OK) {
                ...
                if (!messages.isEmpty()) {
                    ...
                    Samsara.LOGGER.error("Fetching Commit ID");
                    Samsara.commitInfo = fullContent.toString();

                    Pattern pattern = Pattern.compile("`([a-f0-9]{7})`");
                    Matcher matcher = pattern.matcher(fullContent.toString());

                    if (matcher.find()) {
                        Samsara.commitID = matcher.group(1);
                    }

                    Samsara.LOGGER.error("Found Commit ID: " + fullContent.toString());
                } else {
                    Samsara.LOGGER.error("No messages found in the channel.");
                }
            } else {
                Samsara.LOGGER.error("Error: Received HTTP response code " + responseCode);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
