package cc.samsara.protection.auth;
...
public class Client extends WebSocketClient {
    ...
    public void start() {
        try {
            ...
            new HandShakePacket(
                    Samsara.getInstance().getClientProfileMeta().getType().toString(),
                    HwidUtil.getHardwareID(),
                    Samsara.getInstance().getClientProfileMeta().getHash(),
                    Samsara.getInstance().getClientProfileMeta().getVersion()
            ).sendPacket(this);
            ...
            Samsara.LOGGER.error("Network violation 0x02");
    ...
    private void firstThread() {
        ...
        try {
            if (!this.isOpen() && (failedToReconnect || (Flags.didDisconnect && !Flags.didReconnect && Flags.reconnectTime.finished(10000)))) {
                Samsara.LOGGER.error("Network violation 0x03");
            ...
            Samsara.LOGGER.error("Network violation 0x04");
    ...
    private void secondThread() {
        ...
        if (lastReceivedKeepAliveTime.finished(60000)) {
            ...
            Samsara.LOGGER.error("Network violation 0x05");
    ...
    public void onMessage(String encryptedJsonData) {
        try {
            ...
            IPacket packet = Samsara.getInstance().getPacketManager().createPacket(packetId);
            packet.fromJson(decryptedJson);

            switch (packetId) {
                case 1 -> {
                    MessagePacket messagePacket = (MessagePacket) packet;
                    Samsara.getInstance().getEventManager().call(new BackendMessageEvent(messagePacket.getMessage()));
                }
                case 8 -> {
                    ...
                    switch (header) {
                        ...
                        case "userInfo" -> {
                            if (!Flags.isNotAuthenticated) {
                                ...
                                Samsara.getInstance().getComponentManager().registerComponents();
                                Samsara.getInstance().getModuleManager().registerModules();
                                Samsara.getInstance().getCommandManager().registerCommands();
                                Samsara.getInstance().getAnimationManager().start();
                                Samsara.getInstance().getAltConfig().loadConfig();
                                Samsara.getInstance().getDraggableConfig().loadConfig();
                                Samsara.getInstance().getConfigManager().init();
                        ...
                        case "auth" -> {
                            if (msg.equalsIgnoreCase("gud boy") && Flags.authGuiShown) {
                                ...
                                Flags.user.setUid(Samsara.getInstance().getAuthScreen().lastTriedUid);
                                /*Flags.uid = Samsara.getInstance().getAuthScreen().lastTriedUid;*/
                            }
                        }

                        case "bad" -> {
                            if (msg.equalsIgnoreCase("crash")) {
                                Samsara.LOGGER.error("Outdated client version.");
            ...
            e.printStackTrace();
            Samsara.LOGGER.error("Network violation 0x01");
    ...
    public void onError(Exception ex) {
        ...
        if (!Flags.keepAliveWorking ||
            ...
            Samsara.LOGGER.error("Network violation 0x11");
    ...
    public void onClose(int code, String reason, boolean remote) {
        ...
        Flags.reconnectTime.reset();
        //Samsara.LOGGER.warn("Disconnected");
        attemptReconnect();
    }
    ...
    public void attemptReconnect() {
        ...
        if (reconnectAttempts >= 10) {
            failedToReconnect = true;
            Samsara.LOGGER.error("Network violation 0x10");
