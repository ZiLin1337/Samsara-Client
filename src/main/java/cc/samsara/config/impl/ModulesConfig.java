package cc.samsara.config.impl;

import cc.samsara.config.Config;
...
public class ModulesConfig extends Config {
    ...
    public void writeConfig() {
        FileUtil.writeJsonToFile(
                ...
                new File(moduleDirectory, configName + ".samsara").getAbsolutePath()
        );
    }

    @Override
    public void loadConfig() {
        File file = new File(moduleDirectory, configName + ".samsara");
        if (!file.exists()) {
            Samsara.LOGGER.warn("Config file {} not found, skipping load.", file.getAbsolutePath());
            return;
        }
        ...
        try (Reader fr = new FileReader(file);
            ...
            if (root == null || !root.isJsonObject()) {
                Samsara.LOGGER.error("Config {} is not a JSON object. Skipping.", file.getName());
                return;
            }
            ...
            try {
                ...
                Samsara.LOGGER.error("Failed to backup corrupt config: {}", ioEx.getMessage());
            }
            writeConfig();
        } catch (IOException e) {
            Samsara.LOGGER.error("Failed to load config '{}': {}", file.getName(), e.getMessage());
        }
    }
}
