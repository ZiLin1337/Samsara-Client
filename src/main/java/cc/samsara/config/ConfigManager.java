...
import cc.samsara.config.impl.ModulesConfig;
...
public class ConfigManager implements IAccess {
    public final File directory = new File(mc.gameDirectory, "/" + Samsara.NAME.toLowerCase() + "/Configs");
    ...
    public void addConfigsToHashMap() {
        ...
        for (File file : files) {
            if (file.isFile() && file.getName().endsWith(".samsara")) {
                String fileName = file.getName().substring(0, file.getName().length() - ".samsara".length());
    ...
    public void deleteConfig(String configName) {
        ModulesConfig modulesConfig = configs.get(configName);
        if (modulesConfig != null) {
            File configFile = new File(directory, configName + ".samsara");
