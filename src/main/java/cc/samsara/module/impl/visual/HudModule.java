...
import cc.samsara.event.types.Priority;
...
import cc.samsara.util.gif.GifFrameLoadingUtil;
...
import cc.samsara.skija.io.ImageUtil;
import java.awt.*;
...
public class HudModule extends Module {
    ...
    private final ModeProperty watermarkMode = new ModeProperty("Water Mark Mode", "Text", "Text", "Samsara", "Adjust", "Gif", "Sense", "None");
    private final BooleanProperty alternativeColors = new BooleanProperty("Alternative Color", true);
    private final InputProperty clientName = new InputProperty("Client Name", "Samsara");
    private final NumberProperty speed = new NumberProperty("Speed", 100, 1, 300, 1);
    private final FileProperty fileProperty = new FileProperty("Gif", "samsara/dancing-random.gif");
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        if (!(mc.screen instanceof ChatScreen)) {
            String info = "BPS: " + MoveUtil.getBPS();
        ...
        switch (watermarkMode.getProperty()) {
            case "Text": {
                ...
                RenderUtil.drawGradientString(watermarkUIFont, watermarkText, 3, 3, Samsara.getInstance().getFirstColor(), Samsara.getInstance().getSecondColor(), false);
                break;
            }
            ...
            case "Samsara":
                ...
                SkijaUtil.roundedRectangleGradientOutline(
                        ...
                        infoWidth, 18, 3,
                        Samsara.getInstance().getFirstColor(), Samsara.getInstance().getSecondColor(),
                        false, 1.5f, new Color(0, 0, 0, 100)
                );

                uiFont.drawString(info, draggable.getX() + 3.5f, draggable.getY() + 3, Samsara.getInstance().getFirstColor());
                break;
            case "Sense":
                String clientText = "§f" + clientName.getProperty();
                String senseText = "Sense";
                String restText = " §f- (§7" + Samsara.VERSION + "§f) - " + (mc.isLocalServer() ? "none" : censorIfLiquidProxy(mc.getCurrentServer().ip));
                ...
                uiFont.drawString(clientText, 8, 4.5f, Samsara.getInstance().getFirstColor());
                RenderUtil.drawGradientString(uiFont, senseText, 8 + clientWidth, 4.5f, 0f, Samsara.getInstance().getFirstColor(), Samsara.getInstance().getSecondColor(), false);
                uiFont.drawString(restText, 8 + clientWidth + senseWidth, 4.5f, Samsara.getInstance().getFirstColor());
                break;
        }
