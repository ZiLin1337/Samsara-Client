...
import cc.samsara.event.events.impl.input.ModifyMovementEvent;
    ...
    public void onMovementInput(ModifyMovementEvent event) {
        ...
        if (target == null ||
                !Samsara.getInstance().getModuleManager().getModule(SpeedModule.class).isToggled() &&
                        !Samsara.getInstance().getModuleManager().getModule(ElytraFlightModule.class).isToggled() &&
                        !Samsara.getInstance().getModuleManager().getModule(FlightModule.class).isToggled()|| !mc.player.hasLineOfSight(target)
        ) {
            return;
        }
        ...
        LivingEntity targetPosition = Samsara.getInstance().getModuleManager().getModule(KillauraModule.class).target;
        angle += 1;
...
/*        LivingEntity target = Samsara.getInstance().getModuleManager().getModule(KillauraModule.class).target;
        ...
        for (int i = 0; i <= segments; i++) {
            ...
            bufferBuilder.vertex(stack.peek().getPositionMatrix(), (float) (x + xOffset), (float) y, (float) (z + zOffset))
                    .color(Samsara.getInstance().getFirstColor().getRGB());
        }
