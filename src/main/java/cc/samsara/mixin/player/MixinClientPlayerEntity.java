...
import cc.samsara.mixin.accessor.player.ClientPlayerEntityAccessor;
import cc.samsara.event.events.impl.game.*;
import cc.samsara.event.events.impl.input.SprintingTickEndEvent;
...
public abstract class MixinClientPlayerEntity extends AbstractClientPlayer implements IAccess {
    ...
    public void tick(CallbackInfo info) {
        ...
        Samsara.getInstance().getEventManager().call(updateEvent);
    ...
    private Vec3 hookMove(Vec3 movement, MoverType type) {
        ...
        Samsara.getInstance().getEventManager().call(moveEvent);
    ...
    private boolean onItemSlowdown(LocalPlayer player) {
        ...
        Samsara.getInstance().getEventManager().call(slowDownEvent);
    ...
    private void hookSprintingTickEndEvent(CallbackInfo ci) {
        Samsara.getInstance().getEventManager().call(new SprintingTickEndEvent());
    }
    ...
    private void hookPositionSendEvent(CallbackInfo ci) {
        ...
        Samsara.getInstance().getEventManager().call(positionSendEvent);
    ...
    private void hookPositionSendEndEvent(CallbackInfo ci) {
        Samsara.getInstance().getEventManager().call(new PostMotionEvent());
    }
}
